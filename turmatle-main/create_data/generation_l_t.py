...
IMG_DIR = os.path.join(project_root, 'datas/datas_l_t/images_l_t')
LABEL_FILE = os.path.join(project_root, 'datas/datas_l_t/labels_l_t.txt')
...
def get_next_img_idx(img_dir):
    ...
    nums = []
    for f in files:
        if f.startswith('lt') and f.endswith('.png'):
            try:
                num = int(f[2:-4])
                nums.append(num)
            except:
...
def gen_one_lt(img_idx, n_lines, n_triangles, line_start_idx=1, triangle_start_idx=1):
    ...
    screen.update()
    ps_path = os.path.join(IMG_DIR, f'lt{img_idx}.ps')
    png_path = os.path.join(IMG_DIR, f'lt{img_idx}.png')
