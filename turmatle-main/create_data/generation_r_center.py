...
IMG_DIR = os.path.join(project_root, 'datas/datas_r_center/images_r_center')
LABEL_FILE = os.path.join(project_root, 'datas/datas_r_center/labels_r_center.txt')
...
def gen_one_rect_center(img_idx, n_rects, rect_start_idx=1):
    ...
    for i in range(n_rects):
        x1 = round(random.uniform(-3, 0), 1)
        y1 = round(random.uniform(-3, 0), 1)
        x2 = round(random.uniform(0, 3), 1)
        y2 = round(random.uniform(0, 3), 1)
        rect = Rect(Point(x1, y1), Point(x2, y2))
        rect.show()
        descs.append(rect_desc(rect_start_idx + i, (x1, y1), (x2, y2)))
    screen.update()
    ps_path = os.path.join(IMG_DIR, f'r_center{img_idx}.ps')
    png_path = os.path.join(IMG_DIR, f'r_center{img_idx}.png')
...
def get_next_img_idx(img_dir):
    ...
    nums = []
    for f in files:
        if f.startswith('r_center') and f.endswith('.png'):
            try:
                num = int(f[8:-4])  # 去掉 'r_center' 前缀和 '.png' 后缀
                nums.append(num)
            except:
                pass
