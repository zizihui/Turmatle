...
IMG_DIR = os.path.join(project_root, 'datas/datas_t/images_t')
LABEL_FILE = os.path.join(project_root, 'datas/datas_t/labels_t.txt')
...
def gen_one_triangle(img_idx, n_triangles, triangle_start_idx=1):
    ...
    screen.update()
    ps_path = os.path.join(IMG_DIR, f't{img_idx}.ps')
    png_path = os.path.join(IMG_DIR, f't{img_idx}.png')
...
def get_next_img_idx(img_dir):
    ...
    nums = []
    for f in files:
        if f.startswith('t') and f.endswith('.png'):
            try:
                num = int(f[1:-4])
                nums.append(num)
            except:
...
def main():
    ...
    with open(LABEL_FILE, 'a', encoding='utf-8') as f:
        triangle_idx = start_img_idx
        for n_triangles in range(1, 2):
            for j in range(samples_per_group):
                img_path, descs = gen_one_triangle(triangle_idx, n_triangles, triangle_start_idx=1)
