...
IMG_DIR = os.path.join(project_root, 'datas/datas_r_t/images_r_t')
LABEL_FILE = os.path.join(project_root, 'datas/datas_r_t/labels_r_t.txt')
...
def get_next_img_idx(img_dir):
    ...
    nums = []
    for f in files:
        if f.startswith('rt') and f.endswith('.png'):
            try:
                num = int(f[2:-4])
                nums.append(num)
            except:
...
def gen_one_rt(img_idx, n_rects, n_triangles, rect_start_idx=1, triangle_start_idx=1):
    ...
    screen.update()
    ps_path = os.path.join(IMG_DIR, f'rt{img_idx}.ps')
    png_path = os.path.join(IMG_DIR, f'rt{img_idx}.png')
